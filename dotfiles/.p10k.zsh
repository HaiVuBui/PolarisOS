# Generated by Powerlevel10k configuration wizard on 2025-10-31 at 04:13 +07.
...
alias tm='tmux'
alias ta='tmux a'
alias notesync='cd ~/GrandArchive && \
                     git add . && \
                     git commit -m "$(date)" && \
...
note() {
  ...
  if tmux has-session -t "$session" 2>/dev/null; then
    if [ -n "$TMUX" ]; then
      # Already inside tmux → switch to Notes
      tmux switch-client -t "$session"
    else
      # Outside tmux → attach and update default cwd for new windows
      tmux attach-session -t "$session" -c "$dir"
    fi
    return
  fi

  # Session does not exist → create it in GrandArchive and open Polaris.md
  if [ -n "$TMUX" ]; then
    # Inside tmux: create detached, then switch
    tmux new-session -d -s "$session" -c "$dir" "nvim Polaris.md"
    tmux switch-client -t "$session"
  else
    # Outside tmux: create and attach directly (also sets default cwd)
...
# fuzzy navigation
cdf() {
  local base="${1:-$HOME}"    # base directory (default: $HOME)
  local pick
  pick="$(fd -t d -H . "$base" | fzf --height=40% --reverse \
    --prompt='cd → ' --preview 'ls -la {} | head -200')" || return
  [ -n "$pick" ] && cd "$pick"
}
# fuzzy open
fo() {
  local base="${1:-$HOME}"   # base directory (default: $HOME)
  local pick
  pick="$(fd -t f -H . "$base" | fzf --height=40% --reverse \
    --prompt='open → ' --preview 'bat --style=plain --color=always --line-range=:200 {} 2>/dev/null || head -200 {}')" || return
  [ -n "$pick" ] && ${EDITOR:-nvim} "$pick"
}
#fuzzy find
ff() {
  local base="${1:-$HOME}"   # base directory (default: $HOME)
  local pick
  pick="$(fd -t f -H . "$base" | fzf --height=40% --reverse \
    --prompt='find → ' --preview 'bat --style=plain --color=always --line-range=:200 {} 2>/dev/null || head -200 {}')" || return
  [ -n "$pick" ] && echo "$pick"
